...
contract Creature is ERC721Tradable {
    ...
    function buyToken(uint256 _tokenId) public payable{
        require(msg.value>=tokenPrice[_tokenId], "You must pay enough amount to buy it");
        address from = ownerOf(_tokenId);
        address(uint256(from)).transfer(tokenPrice[_tokenId].mul(95).div(100));
        address(uint256(platform)).transfer(tokenPrice[_tokenId].mul(5).div(100));
